...
exports.start = function (SETUP) {
	...
	let STREAM_DISPATCHER;

	const loop_callbacks = [];
	...
	const UpdateEmbed = function () {
		const embed = new Discord.RichEmbed()
			.setAuthor('PADOJRP.net | Server Status', 'https://raw.githubusercontent.com/PADOJRP/Branding/master/Graphics/Logo/Logo.png')
			.setColor(0x002B42)
	...
	const offline = function () {
		...
		const embed = UpdateEmbed()
			.setColor(0xff0000)
			.addField('\n\u200b\nServer Status', ':x: Offline', true)
			.addField('\n\u200b\nOnline Players', '0/64', true);
	...
	const updateMessage = function () {
		...
		for (let i = 0; i < loop_callbacks.length; i++) {
			const callback = loop_callbacks.pop(0);
	...
	bot.on('ready', () => {
		...
		bot.user.setActivity('PADOJRP.net', {'type': 'PLAYING'});
	...
	bot.on('rateLimit', (info) => {
		...
		if (info.path.startsWith(`/channels/${CHANNEL_ID}/messages/${MESSAGE_ID || (MESSAGE ? MESSAGE.id : '')}`)) bot.emit('restart');
		checkLoop().then((running) => {
			...
			if (message.member) {
				if (message.member.hasPermission('ADMINISTRATOR')) {
					if (message.content.startsWith('+status ')) {
						const status = message.content.substr(7).trim();
						const embed = new Discord.RichEmbed()
				...
				if (message.channel.id === BUG_CHANNEL) {
					...
					message.channel.send(embedUser).then(null)
						.catch(console.error);
					bot.channels.get(BUG_LOG_CHANNEL).send(embedStaff)
						.then(null)
						.catch(console.error);
					return message.delete();
				}
